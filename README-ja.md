...
| ワーカー ID レイアウト対応ファクトリ | ✅ `fromLayout()` | ❌ | ❌ |

---

## FAQ

### クライアントに ID を返すときはどうすればいい?

アプリ内では `SnowflakeId` を PHP `int` のまま扱い、**API 境界でのみ string に変換する**。

**JavaScript / TypeScript** — JS の `Number.MAX_SAFE_INTEGER` は `2^53 - 1`。Snowflake ID は `2^62` まで届くため、常に JSON 文字列で送り `BigInt` でパースする：
...
```ts
// TypeScript 側: BigInt でパース
const id = BigInt(response.id);
```

**C# / Unity** — `long` は 64-bit 符号付きで 63-bit ID を完全にカバー：
...
**[Kasumi](https://github.com/niktomo/kasumi) で難読化** — 連番性や内部構造を公開 API コンシューマに見せたくない場合：
...
// エンコード
'id' => (string) $scrambler->scramble($id->value),   // "0yhgff51j5fube"（14文字 base36）

// デコード（対合関数 — 同じ呼び出しで元に戻る）
